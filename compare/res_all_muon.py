...
ay=h.GetYaxis();
ay.SetTitle( "#sigma (p_{T}^{#mu}) / p_{T}^{#mu}" );
...
filename="root/resolution_muon1_"+tag+".txt"
h1=getResolutionGraph(filename)

filename="root/resolution_muon2_"+tag+".txt"
...
b1='%.2f'%( par1[1] )
k1='%.2f'%( par1[2] )
s1="#sqrt{"+a1+" ^{2} /p_{T}^{#mu} +"+b1+" ^{2}/p_{T}^{#mu,2} +"+k1+"^{2}}" 
...
aPerc= a1
kPerc = k1
s1 ="#scale[1.2]{#frac{"+aPerc+"%}{#sqrt{p_{T}^{#mu}}} #oplus "+kPerc+"%}"
print s1
...
aPerc= a1
kPerc = k1
s2 ="#scale[1.2]{#frac{"+aPerc+"%}{#sqrt{p_{T}^{#mu}}} #oplus "+kPerc+"%}"
print s2
