...
Int_t Ana::AnalysisPhotons(vector<LParticle> PhotonsTrue, vector<LParticle> PhotonsReco) {
	...
	for(unsigned int j = 0; j<PhotonsTrue.size(); j++){
		LParticle tele = (LParticle)PhotonsTrue.at(j);
		...
		// reco 
		int indexMatch=-1;
		for(unsigned int i = 0; i<PhotonsReco.size(); i++){
			LParticle rele = (LParticle)PhotonsReco.at(i);
		...
		if (indexMatch>-1){
			LParticle rg = (LParticle)PhotonsReco.at(indexMatch);
			TLorentzVector L1 = rg.GetP();
			float phi = L1.Phi();
			float pt  = L1.Perp();
	...
	if (nevv%nBatch == 0  && nevv>0) {
                cout << "\033[1;31 Training of photons \033[0m\n";
		...
		for (int m=0; m<nBins-1; m++){
			...
			nn=0;
    			cout << "\n  Photons: -> Training for efficiency  = " << m << " sample size=" << finput_photons_eff.size() << endl;
